...
void ffDetectPackagesImpl(FFPackagesResult* result, FFPackagesOptions* options)
{
	...
	FF_STRBUF_AUTO_DESTROY dpkgDir = ffStrbufCreateS("/opt/procursus");
	if (!(options->disabled & FF_PACKAGES_FLAG_DPKG_BIT)) result->dpkg += getNumStrings(&dpkgDir, "/var/lib/dpkg/status", "Status: install ok installed", "dpkg");
